...
#
# Purpose:
#   This script sets up and runs FEAT third-level analysis in FSL for a
#   BIDS-formatted dataset, supporting OLS, FLAME1, or FLAME1plus2 approaches.
#   It can be run interactively (prompt-based) or non-interactively via
#   command-line flags.
#
# Usage:
#   1. Make the script executable:
#      chmod +x third_level_analysis.sh
...
#
# Configuration:
#   - Reads settings from <BIDS_BASE>/code/config/config.yaml if available,
#     including:
#       - bids_version
#       - level-2.fixed_effects_output_dir
#       - level-3.higher_level_output_dir
#     and optional dataset description fields.
#
# Options:
#   - If no valid flags are supplied, the script runs in interactive mode,
#     prompting for directory selections and analysis settings.
#   - Recognized flags (non-interactive mode):
#       --analysis-dir <path>
#       --session <str>
#       --subjects <"sub-...">
#       --mixed-effects <OLS|FLAME1|FLAME1plus2>
#       --z-thresh <float>
#       --cluster-p-thresh <float>
#       --robust
#       --task-name <str>
#       --desc <str>
#       --help
...
#   - create_dataset_description.sh in your scripts folder if you want the
#     dataset_description.json created
#
# Notes:
#   - Outputs go to derivatives/fsl/level-3 (or path read from config.yaml),
#     in a directory named according to approach and optional task/desc labels.
...
# ------------------------------------------------------------------
    ...
    if [ -z "$session_arg" ]; then
        echo "Error: --session is required in this simplified non-interactive approach."
        exit 1
    fi
    ...
    for subj_pattern in "${subject_array[@]}"; do
        for found_subj_path in "$analysis_dir_arg"/$subj_pattern; do
            ...
            session_path="$found_subj_path/$session_arg"
            [ ! -d "$session_path" ] && continue

            gfeat_dirs=($(find "$session_path" -mindepth 1 -maxdepth 1 -type d -name "*.gfeat" | sort))
