...
collect_subjects_and_feat() {
    ...
    for sdir in "${subject_dirs[@]}"; do
        ...
        local session_dir="$sdir/$session_name"
        ...
        if [ "$input_type" == "lower" ]; then
            while IFS= read -r -d $'\0' fdir; do
                dir_list+=("$fdir")
            done < <(find "$session_dir" -type d -name "*.feat" -mindepth 1 -maxdepth 2 -print0 2>/dev/null)
        else
            while IFS= read -r -d $'\0' gdir; do
                dir_list+=("$gdir")
            done < <(find "$session_dir" -type d \( -name "*.gfeat" -o -iname "*fixed-effects*" \) -mindepth 1 -maxdepth 2 -print0 2>/dev/null)
        fi
...
display_confirmed_selections() {
    ...
    for subj in "${sorted_subjects[@]}"; do
        ...
        echo "Subject: $subj | Session: $subj_session"
        echo "----------------------------------------"
...
prompt_for_roi() {
    ...
    if [ ${#all_roi_subdirs[@]} -eq 0 ]; then
        ...
        # We found some ROI subdirectories, so let the user pick
        echo
...
#   Gather all level-3 directories, let user pick one, parse them, etc.
###############################################################################
select_level_3_analysis() {
...
while true; do
    ...
    case "$choice" in
        1)
            ...
            if [ ${#session_names[@]} -eq 0 ]; then
                echo "No session directories found under $SELECTED_ANALYSIS_DIR."
                exit 1
            fi
        ...
        2)
            ...
            if check_level_3_ready; then
                ...
                if [ "$gfeat_choice_num" == "2" ]; then
                    ...
                    mapfile -t session_names < <(find_sessions_in_dir "$SELECTED_ANALYSIS_DIR")
                    if [ ${#session_names[@]} -eq 0 ]; then
                        echo "No sessions found under $SELECTED_ANALYSIS_DIR."
                        exit 1
                    fi
